...
private java.util.List<Modelos.ItemSeleccionable> listaRubros = new java.util.ArrayList<>();
private java.util.List<Modelos.ItemSeleccionable> listaProveedores = new java.util.ArrayList<>();
...
// Proveedor salta a Rubro al dar Enter
dp.cargarComboGenerico(cbxProveedor, 2, cbxRubro);

// Rubro salta al Botón Guardar al dar Enter
dp.cargarComboGenerico(cbxRubro, 1, btnGuardarProducto);
    ...
    private void codigoProductoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_codigoProductoActionPerformed
       DaoProducto dp = new DaoProducto();
        if (dp.existeCodigoBarras(codigoProducto.getText())) {
        new Alerta("El código de barras ya existe", new Color(231, 76, 60), this);
    ...
    private void stockProductoKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_stockProductoKeyPressed
     if (evt.getKeyCode() == java.awt.event.KeyEvent.VK_ENTER) {
        evt.consume(); // Detiene el evento inmediatamente
        
        // Pedimos foco al editor del proveedor
        javax.swing.JTextField editorProv = (javax.swing.JTextField) cbxProveedor.getEditor().getEditorComponent();
        editorProv.requestFocusInWindow();
    ...
    Frame framePrincipal = (window instanceof Frame) ? (Frame) window : null;

    // 1. Abrimos la ventana para registrar un "rubro"
    VentanaFactura diag = new VentanaFactura(framePrincipal, true, this, "rubro");
    diag.setVisible(true);

    // 2. Si el guardado fue exitoso
    if (diag.isAceptado()) {
        // RECARGAMOS el combo de RUBROS (tipo 1) y el foco salta al botón Guardar
        DaoProducto dp = new DaoProducto();
        dp.cargarComboGenerico(cbxRubro, 1, btnGuardarProducto);

        // 3. OBTENEMOS EL NOMBRE que el usuario acaba de registrar
        String nombreNuevo = diag.getNombreGuardado();

        // 4. SELECCIÓN AUTOMÁTICA
        for (int i = 0; i < cbxRubro.getItemCount(); i++) {
            Modelos.ItemSeleccionable item = cbxRubro.getItemAt(i);
            
            // Comparamos el nombre guardado con los que están en el combo recién cargado
            if (item != null && item.getNombre().equalsIgnoreCase(nombreNuevo)) {
                cbxRubro.setSelectedIndex(i); // Selecciona el índice internamente
                cbxRubro.getEditor().setItem(item); // Muestra el texto en la caja editable
                break;
            }
        }
    }
    }//GEN-LAST:event_jButton1ActionPerformed
// Este mapa es la clave: guarda la lista de Rubros bajo la clave "cbxRubro" 
// y la de Proveedores bajo la clave "cbxProveedor"
private String nombreGuardado = "";
  ...
  Window window = javax.swing.SwingUtilities.getWindowAncestor(this);
    ...
    // 2. Si el guardado fue exitoso
    if (diag.isAceptado()) {
        // RECARGAMOS el combo con los datos nuevos de la BD
        DaoProducto dp = new DaoProducto();
        dp.cargarComboGenerico(cbxProveedor, 2, cbxRubro);

        // 3. OBTENEMOS EL NOMBRE que el usuario acaba de escribir
        String nombreNuevo = diag.getNombreGuardado();

        // 4. SELECCIÓN AUTOMÁTICA: Recorremos el combo para encontrarlo
        for (int i = 0; i < cbxProveedor.getItemCount(); i++) {
            Modelos.ItemSeleccionable item = cbxProveedor.getItemAt(i);
            
            // Comparamos el nombre (ignorando mayúsculas/minúsculas)
            if (item != null && item.getNombre().equalsIgnoreCase(nombreNuevo)) {
                cbxProveedor.setSelectedIndex(i); // Lo selecciona visualmente
                
                // IMPORTANTE: Como es un combo editable, hay que forzar el texto en la caja
                cbxProveedor.getEditor().setItem(item); 
                break;
            }
        }
    }
    ...
    private void cbxProveedorKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_cbxProveedorKeyReleased

    }//GEN-LAST:event_cbxProveedorKeyReleased
  ...
  private void limpiarCampos() {
    ...
    // 2. RECARGAR Y REESTABLECER COMBOS (Para limpiar filtros y textos escritos)
    Controlador.DaoProducto dao = new Controlador.DaoProducto();
    
    // RECARGAR RUBROS: Al recargar con el DAO, el modelo vuelve a tener todos los ítems
    dao.cargarComboGenerico(cbxRubro, 1, cbxProveedor); 
    if (cbxRubro.getItemCount() > 0) {
        cbxRubro.setSelectedIndex(0); // Posiciona en "-- Seleccione --"
        cbxRubro.getEditor().setItem(cbxRubro.getItemAt(0)); // Limpia el texto escrito en la caja
    }

    // RECARGAR PROVEEDORES
    dao.cargarComboGenerico(cbxProveedor, 2, cbxRubro);
    if (cbxProveedor.getItemCount() > 0) {
        cbxProveedor.setSelectedIndex(0); // Posiciona en "-- Seleccione --"
        cbxProveedor.getEditor().setItem(cbxProveedor.getItemAt(0)); // Limpia el texto escrito en la caja
    }

    // 3. AUTO-GENERAR EL SIGUIENTE CÓDIGO
    dao.verCodigoDisponible(codigoProducto); 

    // 4. EL FOCO: Preparamos el cursor para el siguiente producto
